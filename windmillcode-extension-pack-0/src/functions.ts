...
export class CreateTaskParams {
  constructor(params:Partial<CreateTaskParams>={}){
    ...
    if(!this.extensionFolder){
      throw new Error("cant seem to find the extension folder as its installed on your computer, make sure no programs are messing with the extension location or try to reinstall, if you are a beginner you might need a reinstall")
    }
    if(!this.executable){
  ...
  extensionFolder = vscode.extensions.getExtension('windmillcode-publisher-0.windmillcode-extension-pack-0')?.extensionPath
  executable!:string
